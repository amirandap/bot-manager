...
export const formatMessage = (message: string, user: User) => {
  const { cleanedPhoneNumber, isValid } = cleanAndFormatPhoneNumber(user.celular || fallbackNumber);
...
export const sendMessage = async (client: Client | null, phoneNumber: string , message: string ) => {
...
export const sendErrorMessage = async (client: Client | null, message: string) => {
