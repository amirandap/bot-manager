...
router.get('/', async (req, res) => {
  try {
    ...
    const groupChats = chats.filter(chat => chat.id._serialized.endsWith('@g.us')) as GroupChat[];
    console.log(`GET /get-groups: ${groupChats.length} group chats found`);

    const groups = await Promise.all(groupChats.map(group => getGroupDetails(group)));
    ...
    return res.status(500).send({ error: `Error retrieving groups: ${reason}` });
  }
});
