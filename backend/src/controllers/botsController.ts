...
export class BotsController {
  ...
  /**
   * GET /api/bots/template - Get current bot template configuration
   */
  public async getBotTemplate(req: Request, res: Response): Promise<void> {
    try {
      const template = this.botSpawnerService.getBotTemplate();
      res.json({
        template,
        message: "Bot template retrieved successfully",
      });
    } catch (error) {
  ...
  /**
   * PUT /api/bots/template - Update bot template configuration
   */
  public async updateBotTemplate(req: Request, res: Response): Promise<void> {
    try {
      const { configPath, value } = req.body;

      if (!configPath || value === undefined) {
        res.status(400).json({ 
          error: "configPath and value are required",
          example: {
            configPath: "pm2Config.memory.maxMemoryRestart",
            value: "512M"
          }
        });
        return;
      ...
      const success = this.botSpawnerService.updateBotTemplate(configPath, value);

      if (success) {
        res.json({
          success: true,
          message: `Template configuration updated: ${configPath} = ${value}`,
          note: "Restart bots to apply changes"
        });
      } else {
      ...
      res.status(500).json({ error: "Failed to update bot template" });
    }
  }

  /**
   * POST /api/bots/template/memory - Update memory configuration for all bots
   */
  public async updateMemoryConfig(req: Request, res: Response): Promise<void> {
    try {
      const { maxMemoryRestart, nodeMaxOldSpace } = req.body;

      if (!maxMemoryRestart || !nodeMaxOldSpace) {
        res.status(400).json({ 
          error: "maxMemoryRestart and nodeMaxOldSpace are required",
          example: {
      ...
      const success = await this.botSpawnerService.updateMemoryConfig(
        maxMemoryRestart, 
        nodeMaxOldSpace
      );
      ...
      if (success) {
        res.json({
          ...
          configuration: {
            maxMemoryRestart,
            nodeMaxOldSpace: `${nodeMaxOldSpace}MB`
          },
          note: "Restart bots to apply changes"
        });
      } else {
  ...
  /**
   * POST /api/bots/template/apply - Apply current template to all existing bots
   */
  public async applyTemplateToAllBots(req: Request, res: Response): Promise<void> {
    try {
      console.log("🔄 Applying template configuration to all existing bots...");
      
      const result = await this.botSpawnerService.applyTemplateToExistingBots();

      if (result.success) {
        res.json({
          success: true,
          message: "Template applied to all bots successfully",
          restartedBots: result.restartedBots,
          count: result.restartedBots.length
        });
      } else {
        res.status(500).json({ 
          error: "Failed to apply template to some bots",
          restartedBots: result.restartedBots
        });
      }
    } catch (error) {
      console.error("Error applying template to bots:", error);
      res.status(500).json({ error: "Failed to apply template to bots" });
    }
  }

  /**
   * POST /api/bots/template/reload - Reload template configuration from file
   */
  public async reloadBotTemplate(req: Request, res: Response): Promise<void> {
    try {
      this.botSpawnerService.reloadBotTemplate();
      const template = this.botSpawnerService.getBotTemplate();
      
      res.json({
        success: true,
        message: "Bot template reloaded successfully",
        template
      });
    } catch (error) {
      console.error("Error reloading bot template:", error);
      res.status(500).json({ error: "Failed to reload bot template" });
    }
  }
}
